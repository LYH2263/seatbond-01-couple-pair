...
type Cell = { row: number; col: number; is_aisle: boolean; occupied: boolean; heat: number; couple: boolean };
  ...
  return (
    <>
      <div className="toolbar">
        ...
        {map && (
          <span className="mono">
            {map.hall_name} · {map.rows}×{map.cols} · 热力座图 · ♥ 情侣对
          </span>
        )}
      </div>
      ...
      {map && (
        <div className="seat-grid" style={gridStyle}>
          {map.cells.map((c) => (
            <div
              key={`${c.row}-${c.col}`}
              className={`seat ${c.is_aisle ? "aisle" : c.occupied ? "occ" : "free"}${c.couple ? " couple" : ""}`}
              title={`R${c.row}C${c.col}${c.couple ? " · 情侣对" : ""}`}
              style={
                !c.is_aisle && c.heat
